...
chrome.storage.local.get('tab_modifier', function (items) {
    ...
    var tab_modifier = items.tab_modifier, todos = [], processPage;
    ...
    processPage = function () {
        ...
        for (var i = 0; i < tab_modifier.rules.length; i++) {
            if (tab_modifier.rules[i].detection === undefined || tab_modifier.rules[i].detection === 'CONTAINS') {
                if (location.href.indexOf(tab_modifier.rules[i].url_fragment) !== -1) {
                    todos.push(tab_modifier.rules[i]);
                }
            } else {
		...
		var observer_icon = null;
		var observer_title = null;
		...
		for (var i = 0; i < todos.length; i++) {
			...
			if (rule.tab.title !== null) {
				if (document.title !== null) {
					document.title = processTitle(rule, location.href, document.title);
				}
			}

			//new scope for title_change_by_me
			(function (){
			...
			if (rule.tab.icon !== null) {
				...
				//new scope for icon_changed_by_me
				(function (){
		...
		// Observe when the website has changed the title; we only use the last-created one
		if (observer_title !== null && document.querySelector('head > title') !== null) {
		...
		if (observer_icon !== null && document.querySelector('head link[rel*="icon"]') !== null) {
